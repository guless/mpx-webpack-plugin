        ...
        compiler.hooks.make.tapAsync(PLUGIN_NAME, ( compilation, callback ) => {
            Promise.resolve(this.entry()).then(( entry ) => {
                if ( typeof entry == "string" || Array.isArray(entry) ) {
                    const dependency = new MPXMultiEntryDependency(this.entry, this.name);
                    ...
                    /// 添加 App 入口点。
                    const dependencies = [{ "name": this.name, "dependency": new MPXMultiEntryDependency(entry[this.name], this.name) }];
